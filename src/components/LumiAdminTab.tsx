...
export const LumiAdminTab: React.FC<LumiAdminTabProps> = ({
  ...
  // Sub-tabs in Admin
  const [subTab, setSubTab] = useState<'prompts' | 'faqs' | 'hours' | 'policies' | 'contact' | 'resources'>('prompts');
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden relative">
      ...
      <div className="bg-slate-100 border-b border-slate-200 px-4 flex gap-1 overflow-x-auto">
        ...
        >
          ...
          <span>{language === 'th' ? 'ทรัพยากร' : 'Resources'}</span>
          <span className="px-1.5 py-0.2 bg-blue-100 text-blue-900 text-[10px] rounded-full">
            {adminData.resources?.length || 0}
          </span>
        </button>
      </div>
          ...
          {subTab === 'resources' && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div>
                  <h4 className="text-sm font-bold text-slate-800 font-['Prompt',sans-serif]">
                    {language === 'th' ? 'จัดการทรัพยากรห้องสมุด (Library Resources)' : 'Library Resources Management'}
                  </h4>
                  <p className="text-xs text-slate-500">
                    {language === 'th' ? 'ลิงก์และคำอธิบายทรัพยากรห้องสมุดออนไลน์ที่ใช้ในแท็บทรัพยากร' : 'Online resources shown in the Resources tab'}
                  </p>
                </div>
                <button
                ...
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>{language === 'th' ? 'เพิ่มทรัพยากรใหม่' : 'Add Resource'}</span>
                </button>
              </div>
              ...
              {/* Resources List */}
              <div className="space-y-2.5">
                {(adminData.resources || []).map((resource, idx) => (
                  <div
                    key={resource.id}
                    className="p-4 bg-white rounded-xl border border-slate-200/90 shadow-2xs space-y-2"
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0 flex-col sm:flex-row">
                        ...
                        {idx < (adminData.resources?.length || 0) - 1 && (
                          <button
                ...
                {(!adminData.resources || adminData.resources.length === 0) && (
                  <div className="p-6 bg-white rounded-xl border border-dashed border-slate-200 text-center space-y-2">
                    <Database className="w-8 h-8 text-slate-300 mx-auto" />
                    <p className="text-xs text-slate-500 font-medium">
                      {language === 'th' ? 'ยังไม่มีทรัพยากรใดๆ' : 'No resources yet'}
                    </p>
                    <button
                      type="button"
                      onClick={handleOpenAddResource}
                    ...
                    >
                      <Plus className="w-3.5 h-3.5" />
                      <span>{language === 'th' ? 'เพิ่มทรัพยากรแรก' : 'Add First Resource'}</span>
                    </button>
                  </div>
                )}
              </div>
      ...
      {isResourceModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-5 sm:p-6 shadow-xl border border-slate-200 space-y-4 animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h4 className="text-sm sm:text-base font-bold text-slate-800 font-['Prompt',sans-serif]">
                {editingResource
                  ? language === 'th'
            ...
            <form onSubmit={handleSaveResource} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">
                  {language === 'th' ? 'ชื่อทรัพยากร (ไทย)*' : 'Resource Title (TH)*'}
                </label>
                <input
                  type="text"
                  required
                  value={resourceForm.titleTh}
                  onChange={(e) => setResourceForm({ ...resourceForm, titleTh: e.target.value })}
                  placeholder="เช่น Web OPAC"
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
              </div>
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">
                  {language === 'th' ? 'ชื่อทรัพยากร (อังกฤษ)' : 'Resource Title (EN)'}
                </label>
                <input
                  type="text"
                  value={resourceForm.titleEn}
                  onChange={(e) => setResourceForm({ ...resourceForm, titleEn: e.target.value })}
                  placeholder="e.g. Web OPAC"
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800"
                />
              </div>
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">
                  {language === 'th' ? 'คำอธิบาย (ไทย)*' : 'Description (TH)*'}
                </label>
                <textarea
                  required
                  value={resourceForm.descTh}
                  onChange={(e) => setResourceForm({ ...resourceForm, descTh: e.target.value })}
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">
                  {language === 'th' ? 'คำอธิบาย (อังกฤษ)' : 'Description (EN)'}
                </label>
                <textarea
                  value={resourceForm.descEn}
                  onChange={(e) => setResourceForm({ ...resourceForm, descEn: e.target.value })}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setResourceForm({ ...resourceForm, icon: e.target.value as any })}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800"
                >
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    type="text"
                    value={resourceForm.categoryTh}
                    onChange={(e) => setResourceForm({ ...resourceForm, categoryTh: e.target.value })}
                    placeholder="เช่น ระบบสืบค้น"
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-800"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={resourceForm.categoryEn}
                    onChange={(e) => setResourceForm({ ...resourceForm, categoryEn: e.target.value })}
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setIsResourceModalOpen(false)}
                  className="flex-1 px-4 py-2 border border-slate-200 hover:border-slate-300 text-slate-700 rounded-lg text-xs font-semibold transition-colors"
                >
                  {language === 'th' ? 'ยกเลิก' : 'Cancel'}
                </button>
                <button
                  type="submit"
                  className="flex-1 px-4 py-2 bg-blue-900 hover:bg-blue-800 text-white rounded-lg text-xs font-semibold transition-colors shadow-xs cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <Save className="w-3.5 h-3.5" />
